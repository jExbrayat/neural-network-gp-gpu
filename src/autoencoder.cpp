...
void Autoencoder::fit(const xarray<float> &x_train, const unsigned int &epochs, const int &batch_size, const float &learning_rate)
{
...
tuple<xarray<float>, xarray<float>, xarray<float>, xarray<float>> Autoencoder::load_mnist_dataset(const float &train_test_split) {
        ...
        xarray<float> x_train = transform_mnist_images(dataset.training_images, {dataset.training_images.size(), 784}); // shape (N, 784)
        xarray<float> y_train = transform_mnist_labels(dataset.training_labels, {dataset.training_labels.size(), 1}); // shape (N, 1)
        xarray<float> x_test = transform_mnist_images(dataset.test_images, {dataset.test_images.size(), 784});
        xarray<float> y_test = transform_mnist_labels(dataset.test_labels, {dataset.test_labels.size(), 1});
